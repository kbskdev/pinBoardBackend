...
exports.getPublicCompList = async(req,res,next)=>{
    try{
        const compositionList = await UserModel.User.aggregate([{$match:{_id:mongoose.Types.ObjectId(req.params.user),'composition.public':'public'}},{$project:{'composition.$':1}}])
        res.status(200).json({
            status:'success',
