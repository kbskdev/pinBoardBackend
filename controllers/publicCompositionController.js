...
exports.getOneCompPublic = async(req,res,next)=>{
    try{
        const imageList = await UserModel.User.findOne({_id:req.params.user,'composition.$[id].public':"public"},
            {_id:0,'composition.$':1},{arrayFilters:[{'id._id':req.params.composition}]})
        res.status(200).json({
            status:'success',
            data:imageList
        })
